...
def importer_submit(request):
  ...
  if destination['ouputFormat'] == 'index':
    ...
    job_handle = _envelope_job(request, source, index_name, start_time=start_time, lib_path=destination['indexerJobLibPath'])
  else:
...
def _envelope_job(request, file_format, collection_name, query=None, start_time=None, lib_path=None):
  ...
  morphline = indexer.generate_config(collection_name, file_format, unique_field, lib_path=lib_path)
