...
from gcp.gcs import join, parse_uri
from gcp.gcs.gcsfs import GCSFileSystem
from gcp.gcs.gcstest_utils import GCSTestBase, generate_id


class GCSFSTest(GCSTestBase):
  @classmethod
  def setUpClass(cls):
    GCSTestBase.setUpClass()
    if not cls.shouldSkip():
      cls.fs = GCSFileSystem(cls.gcs_connection)
  ...
  def test_exists(self):
    ...
    assert_true(self.fs.exists('gs://%s' % self.bucket_name))
    assert_true(self.fs.exists('gs://'))
    fake_bucket = 'fake%s' % generate_id(8, string.ascii_lowercase + string.digits)
    assert_false(self.fs.exists('gs://%s' % fake_bucket))
  ...
  def test_stats(self):
    ...
    root_stat = self.fs.stats('gs://')
    eq_(True, root_stat.isDir)
    eq_('gs://', root_stat.path)

    bucket_stat = self.fs.stats('gs://%s' % self.bucket_name)
    eq_(True, bucket_stat.isDir)
    eq_('gs://%s' % self.bucket_name, bucket_stat.path)
